...
namespace hydrogen_fem {
    ...
    double Hydrogen_FEM::rho(double r) const
    {
        auto klo = 0U;
        auto const max = static_cast<std::uint32_t>(NODE_TOTAL - 1);
        auto khi = max;
        ...
        auto const tmp = (phi_[khi] - phi_[klo]) / (node_r_glo_[khi] - node_r_glo_[klo]) * (r - node_r_glo_[klo]) + phi_[klo];
        return tmp * tmp;
    }
